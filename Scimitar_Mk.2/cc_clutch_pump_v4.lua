...
local function engine_clutch()
    local engine_sides = {}
    for _, side in pairs(SIDES) do
        local side_type = peripheral.getType(side)
        if contains(side_type, FLUID_TARGET_TYPES) then
            table.insert(engine_sides, side)
        end
    end

    while true do
        for _, engine_side in pairs(engine_sides) do
            rs.setOutput(engine_side, not rs.getInput(REDSTONE_INPUT))
        end
        sleep()
    end
end
...
parallel.waitForAny(refueling, engine_clutch)
